...
serve(async (req) => {
  ...
  try {
    ...
    const apiKey =
      paymentSettings.environment === "production"
        ? paymentSettings.asaas_api_key_production
        : paymentSettings.asaas_api_key_sandbox;

    if (!apiKey) {
      throw new Error(
        `Chave API do Asaas não configurada para ambiente ${paymentSettings.environment}. Configure no painel administrativo.`,
      );
    }
    ...
    if (!paymentResponse.ok) {
      ...
      const errorMessage =
        payment.errors?.[0]?.description || payment.message || `Payment creation failed (${paymentResponse.status})`;
      throw new Error(errorMessage);
    }
    ...
    if (!qrCodeResponse.ok) {
      ...
      await supabaseClient.from("pre_enrollments").update({ status: "pending_payment" }).eq("id", pre_enrollment_id);
